...
export default class QQZoneShare extends Component {
  ...
  shareText() {
    QQ.shareText("这是一段分享文字",QQ.shareScene.QQZone)
      .then((result)=>{Alert.alert('QQZone分享结果',''+result);})
      .catch((error)=>{Alert.alert('QQZone分享结果',''+error);});
  }
  shareImage() {
  ...
  shareNews() {
    QQ.shareNews('https://facebook.github.io/react-native/',resolveAssetSource(require('./news.jpg')).uri,'分享新闻的标题','分享新闻的描述',QQ.shareScene.QQZone)
    .then((result)=>{Alert.alert('QQZone分享结果',''+result);})
    .catch((error)=>{Alert.alert('QQZone分享结果',''+error);});
  }
  shareAudio() {
