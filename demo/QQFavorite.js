...
export default class QQZoneShare extends Component {
  ...
  shareText() {
    QQ.shareText("这是一段收藏文字",QQ.shareScene.Favorite)
      .then((result)=>{Alert.alert('QQ收藏结果',''+result);})
      .catch((error)=>{Alert.alert('QQ收藏结果',''+error);});
  }
  shareImage() {
  ...
  shareNews() {
    QQ.shareNews('https://facebook.github.io/react-native/',resolveAssetSource(require('./news.jpg')).uri,'收藏新闻的标题','收藏新闻的描述',QQ.shareScene.Favorite)
    .then((result)=>{Alert.alert('QQ收藏结果',''+result);})
    .catch((error)=>{Alert.alert('QQ收藏结果',''+error);});
  }
  shareAudio() {
