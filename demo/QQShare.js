...
export default class QQShare extends Component {
  ...
  shareText() {
    QQ.shareText("这是一段分享文字",QQ.shareScene.QQ)
      .then((result)=>{Alert.alert('QQ分享结果',''+result);})
      .catch((error)=>{Alert.alert('QQ分享结果',''+error);});
  }
  shareImage() {
  ...
  shareImageFromLocal() {
    CameraRoll.getPhotos({first:1,assetType:'Photos'})
              .then((asset) => {console.log(asset.edges[0].node.image.uri);
  ...
  shareNews() {
    QQ.shareNews('https://facebook.github.io/react-native/',resolveAssetSource(require('./news.jpg')).uri,'分享新闻的标题','分享新闻的描述',QQ.shareScene.QQ)
    .then((result)=>{Alert.alert('QQ分享结果',''+result);})
    .catch((error)=>{Alert.alert('QQ分享结果',''+error);});
  }
  shareAudio() {
