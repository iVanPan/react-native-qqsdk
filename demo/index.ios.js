/**
 * Sample React Native App
 * https://github.com/facebook/react-native
 * @flow
 */

import React, { Component } from 'react';
import {
  AppRegistry,
  StyleSheet,
  Text,
  Alert,
  Button,
  View
} from 'react-native';
import * as QQ from "react-native-qqsdk";
import resolveAssetSource from 'resolveAssetSource';
export default class Test extends Component {
  render() {
    console.log('imageType is ',resolveAssetSource(require('./news.jpg')));
    const imgUrl = "http://ww3.sinaimg.cn/mw690/687520b6jw1faic5ciy9aj20u011had3.jpg";
    const audioPreviewUrl = "http://music.163.com/#/m/song?id=3986326";
    const audioUrl = "http://m8.music.126.net/20161208171916/e054a2158eb33e52e4944b1bf55d744c/ymusic/3c60/69de/c24f/554744c158908dcad5a8abc4973fb98b.mp3";
    const base64 = "";
    return (
      <View style={styles.container}>
        <Text style={styles.welcome}>
          Welcome to React Native!
        </Text>
        <Text style={styles.instructions}>
          To get started, edit index.ios.js
        </Text>
        <Button
          onPress={()=>{QQ.isQQClientInstalled().then(()=>{console.log('Installed is', QQ.imageType,QQ.shareScene)}).catch(()=>{console.log('nothing',QQ.imageType,QQ.shareScene)});}}
          title="checkClientInstalled"
          color="#841584"
        />
        <Button
          onPress={()=>{QQ.ssoLogin().then((result)=>{console.log('result is', result)}).catch((code)=>{console.log('error is', code)});}}
          title="Login"
          color="#841584"
        />
        <Button
          onPress={()=>{QQ.shareImage(imgUrl,QQ.imageType.Network,'分享图片','分享结果',QQ.shareScene.QQ).then((result)=>{console.log('result is', result)}).catch((code)=>{console.log('error is', code)});}}
          title="shareNetworkImageToQQ"
          color="#841584"
        />
        <Button
          onPress={()=>{QQ.shareImage(base64,QQ.imageType.Base64,'分享图片','分享结果',QQ.shareScene.QQ).then((result)=>{console.log('result is', result)}).catch((code)=>{console.log('error is', code)});}}
          title="shareBase64ImageToQQ"
          color="#841584"
        />
        <Button
          onPress={()=>{QQ.shareImage(resolveAssetSource(require('./news.jpg')).uri,QQ.imageType.Local,'分享图片','分享结果',QQ.shareScene.QQ).then((result)=>{console.log('result is', result)}).catch((code)=>{console.log('error is', code)});}}
          title="shareLocalImageToQQ"
          color="#841584"
        />
        <Button
          onPress={()=>{QQ.shareNews('http://www.cnbeta.com/',resolveAssetSource(require('./news.jpg')).uri,QQ.imageType.Local,'分享新闻','新闻分享结果',QQ.shareScene.QQ).then((result)=>{console.log('result is', result)}).catch((code)=>{console.log('error is', code)});}}
          title="shareNewsWithLocalImageToQQ"
          color="#841584"
        />
        <Button
          onPress={()=>{QQ.shareNews('http://www.cnbeta.com/',base64,QQ.imageType.Base64,'分享新闻','新闻分享结果',QQ.shareScene.QQ).then((result)=>{console.log('result is', result)}).catch((code)=>{console.log('error is', code)});}}
          title="shareNewsWithBaseImageToQQ"
          color="#841584"
        />
        <Button
          onPress={()=>{QQ.shareNews('http://www.cnbeta.com/',imgUrl,QQ.imageType.Network,'分享新闻','新闻分享结果',QQ.shareScene.QQ).then((result)=>{console.log('result is', result)}).catch((code)=>{console.log('error is', code)});}}
          title="shareNewsWithNetworkImageToQQ"
          color="#841584"
        />
        <Button
          onPress={()=>{QQ.shareAudio(audioPreviewUrl,audioUrl,resolveAssetSource(require('./news.jpg')).uri,QQ.imageType.Local,'分享新闻','新闻分享结果',QQ.shareScene.QQ).then((result)=>{console.log('result is', result)}).catch((code)=>{console.log('error is', code)});}}
          title="shareAudioWithLocalImageToQQ"
          color="#841584"
        />
        <Button
          onPress={()=>{QQ.shareAudio(audioPreviewUrl,audioUrl,base64,QQ.imageType.Base64,'分享新闻','新闻分享结果',QQ.shareScene.Favorite).then((result)=>{console.log('result is', result)}).catch((code)=>{console.log('error is', code)});}}
          title="shareAudioWithBaseImageToQQ"
          color="#841584"
        />
        <Button
          onPress={()=>{QQ.shareAudio(audioPreviewUrl,audioUrl,imgUrl,QQ.imageType.Network,'分享新闻','新闻分享结果',QQ.shareScene.QQZone).then((result)=>{console.log('result is', result)}).catch((code)=>{console.log('error is', code)});}}
          title="shareAudioWithNetworkImageToQQ"
          color="#841584"
        />
        <Button
          onPress={()=>{QQ.logout().then((result)=>{console.log('result is', result)}).catch((code)=>{console.log('error is', code)});}}
          title="Logout"
          color="#841584"
        />
        <Button
          onPress={()=>{QQ.shareText("分享文字",QQ.shareScene.QQ).then((result)=>{console.log('result is', result)}).catch((code)=>{console.log('error is', code)});}}
          title="shareTextToQQ"
          color="#841584"
        />

      </View>
    );
  }
}
const styles = StyleSheet.create({
  container: {
    flex: 1,
    justifyContent: 'center',
    alignItems: 'center',
    backgroundColor: '#F5FCFF',
  },
  welcome: {
    fontSize: 20,
    textAlign: 'center',
    margin: 10,
  },
  instructions: {
    textAlign: 'center',
    color: '#333333',
    marginBottom: 5,
  },
});

AppRegistry.registerComponent('Test', () => Test);
